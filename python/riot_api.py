...
class RiotAPI:
    ...
    def for_region(self, platform: str, routing: str) -> "RiotAPI":
        """Return a lightweight regional client sharing throttling and HTTP state."""
        regional = object.__new__(RiotAPI)
        regional.platform = platform.strip().lower()
        regional.routing = routing.strip().lower()
        regional.timeout = self.timeout
        regional.max_retries = self.max_retries
        regional.on_retry = self.on_retry
        regional.session = self.session
        regional._blocked_until = self._blocked_until
        return regional
