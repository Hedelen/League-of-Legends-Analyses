...
def init_database(settings: Any) -> None:
    ...
    with connect(settings.postgres_dsn) as conn:
        ...
        if nested_columns:
            names = ", ".join(
                f"{row['table_name']}.{row['column_name']}" for row in nested_columns
            )
            raise RuntimeError(f"Analyst ERD still contains nested columns: {names}")
    print(
        "[setup] Relational public ERD is ready: no JSON/array columns; "
...
def main() -> int:
    ...
    if args.command == "smoke-discovery":
        ...
        print(
            f"[smoke] Multi-region OP.GG Kayle IDs -> Riot validation; "
            f"patches={', '.join(window.patches)}"
        )
        for report in reports:
            print(
                f"  {report['riot_id']} [{report['platform']}/{report['routing']}]: "
                f"{report['result']}; "
    ...
    with connect(settings.postgres_dsn) as conn:
        ...
        if run["status"] == "DISCOVERING":
            discovery_phase(conn, api, run, settings)
            run = get_run(conn, int(run["run_id"]))
        collection_phase(conn, api, run, settings)
        print_status(conn, int(run["run_id"]))
    return 0
